...
void BancThread::ThreadFuncHelper() {
    MarkThreadReady();
...
void BancThread::exception_caught(){
    exception_caught_ = true;
}

bool BancThread::is_exception_free(){
    return !exception_caught_;
}

void BancThread::ThreadCleanupHookHelper() {

}

void BancThread::init(){
    cl::print_line("odo : init");
    StartThread();
    WaitForThreadReady();
    cl::print_line("odo : init done");
}
void BancThread::cleanup(){
    ex_ptr_ = NULL;
    TerminateThread();
}


void BancThread::pushBothPixmap(std::unique_ptr<QPixmap> qpixmap, std::unique_ptr<QPixmap> qpixmap2){
...
std::vector<std::unique_ptr<QPixmap>> BancThread::getBothPixmap(){
    if(ex_ptr_)
